...
sudo rm -r target
TIMESTAMP=$(date +%s)
CONSE_PANEL_CONTAINER_ID=$(docker container ls  | grep 'conse-panel-pg' | awk '{print $1}')
sudo docker build -t conse-panel-pg-$TIMESTAMP -f $(pwd)/infra/docker/panel/postgres/Dockerfile . --no-cache
sudo docker stop $CONSE_PANEL_CONTAINER_ID
...
sudo docker rm $CONSE_PANEL_CONTAINER_ID
