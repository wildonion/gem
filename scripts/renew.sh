...
sudo docker run -d -it -p 80:80 -p 443:443 -v $(pwd)/infra/data/nginx/confs/:/etc/nginx -v $(pwd)/infra/data/nginx/wwws/:/usr/share/nginx/ -v $(pwd)/infra/assets/:/etc/nginx/assets -v $(pwd)/infra/logs/:/etc/nginx/logs --name nginx --network host nginx
