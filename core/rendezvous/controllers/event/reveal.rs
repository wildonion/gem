...
// -------------------------------------------------------------------------
pub async fn role(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
                                    ...
                                    if misc::event_belongs_to_god(_id.unwrap(), event_id, db_to_pass.clone()).await || access_level == DEV_ACCESS{ // checking that the passed in event id is belongs to the passed in god id or not 
                                        ...
                                        match events.find_one(doc! { "_id": event_id, "is_expired": false }, None).await.unwrap(){ // getting a none expired event
                                            Some(event_doc) => {
                                                ...
                                                let response_body = misc::app::Response::<schemas::event::RevealEventInfo>{ // we have to specify a generic type for data field in Response struct which in our case is EventInfo struct
