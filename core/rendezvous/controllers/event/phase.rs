...
pub async fn insert(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{ // inserting a new passed event for the current event during the game
                                            ...
                                            Some(event_info) => {
                                                ...
                                                match events.find_one_and_update(doc!{"_id": event_id}, doc!{"$set": {"phases": serialized_updated_phases, "updated_at": Some(now)}}, Some(update_option)).await.unwrap(){
                                                    Some(event_doc) => {
                                                        ...
                                                        let response_body = misc::app::Response::<schemas::event::InsertPhaseResponse>{ // we have to specify a generic type for data field in Response struct which in our case is InsertPhaseResponse struct
