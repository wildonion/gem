...
pub async fn explore_none_expired_events(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// --------------------------------------------------------------------------------------
pub async fn player_all_expired(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// --------------------------------------------------------------------------------------
pub async fn player_all_for_dev(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
        ...
        Ok((token_data, req)) => { // the decoded token and the request object will be returned from the function call since the Copy and Clone trait is not implemented for the hyper Request and Response object thus we can't have the borrowed form of the req object by passing it into the pass() function therefore it'll be moved and we have to return it from the pass() function   
            ...
            if middlewares::auth::user::exists(Some(&db_to_pass), _id, access_level).await{ // finding the user with these info extracted from jwt
                if access_level == DEV_ACCESS { // NOTE - only dev can handle this route
                    ...
                    let player_id = format!("{}", req.param("playerId").unwrap()); 
                    let player_object_id = ObjectId::parse_str(player_id.as_str()).unwrap();

                    ////////////////// DB Ops
                    
                    let filter = doc! { "players._id": player_object_id }; // filtering all expired events
                    let events = db.database(&db_name).collection::<schemas::event::EventInfo>("events"); // selecting events collection to fetch and deserialize all event infos or documents from BSON into the ExploreEventInfo struct
                    let mut all_expired_events = schemas::event::AvailableEvents{
                        events: vec![],
                    };
...
// ➝ Return : Hyper Response Body or Hyper Error
// --------------------------------------------------------------------------------------
pub async fn player_all_none_expired(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn all_none_expired(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn all_expired(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn single(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn god_single(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
...
// -------------------------------- get god events controller
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn god_all(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
            ...
            if middlewares::auth::user::exists(Some(&db_to_pass), _id, access_level).await{ // finding the user with these info extracted from jwt
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS{ // NOTE - only dev and admin (God) can handle this route
                    ...
                    if all_god_events.is_empty(){
                        ...
                        let response_body = misc::app::Response::<Vec<schemas::event::EventInfo>>{
                            message: FETCHED,
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn god_all_for_dev(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
        ...
        Ok((token_data, req)) => { // the decoded token and the request object will be returned from the function call since the Copy and Clone trait is not implemented for the hyper Request and Response object thus we can't have the borrowed form of the req object by passing it into the pass() function therefore it'll be moved and we have to return it from the pass() function   
            ...
            if middlewares::auth::user::exists(Some(&db_to_pass), _id, access_level).await{ // finding the user with these info extracted from jwt
                if access_level == DEV_ACCESS{ // NOTE - only dev and admin (God) can handle this route
                    ...
                    let god_id = format!("{}", req.param("godId").unwrap());
...
// ➝ Return : Hyper Response Body or Hyper Error
// ----------------------------------------------------------------------------------------------
pub async fn group_all(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
