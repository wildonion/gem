...
// -------------------------------- lock event controller
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn main(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
                ...
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS{ // NOTE - only dev and admin (God) can handle this route
                    ...
                    match serde_json::from_reader(whole_body_bytes.reader()){ // read the bytes of the filled buffer with hyper incoming body from the client by calling the reader() method from the Buf trait
                        Ok(value) => { // making a serde value from the buffer which is a future IO stream coming from the client
                            ...
                            match serde_json::from_str::<schemas::event::LockEventRequest>(&json){ // the generic type of from_str() method is LockEventRequest struct - mapping (deserializing) the json string into the LockEventRequest struct
                                Ok(exp_info) => {
                                    ...
                                    if misc::event_belongs_to_god(_id.unwrap(), event_id, db_to_pass.clone()).await || access_level == DEV_ACCESS{ // checking that the passed in event id is belongs to the passed in god id or not 
                                        ...
                                        match events.find_one_and_update(doc!{"_id": event_id}, doc!{"$set": {"is_locked": true, "updated_at": Some(Utc::now().timestamp())}}, Some(update_option)).await.unwrap(){ // finding event based on event id
                                            Some(event_doc) => { // deserializing BSON into the EventInfo struct
                                                let response_body = misc::app::Response::<schemas::event::EventInfo>{ // we have to specify a generic type for data field in Response struct which in our case is EventInfo struct
                                                    data: Some(event_doc),
