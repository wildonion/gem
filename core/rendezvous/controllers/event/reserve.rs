...
// -------------------------------------------------------------------------
pub async fn mock_reservation(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
                        ...
                        Ok(value) => { // making a serde value from the buffer which is a future IO stream coming from the client
                            ...
                            match serde_json::from_str::<schemas::event::MockReservationRequest>(&json){ // the generic type of from_str() method is MockReservationRequest struct - mapping (deserializing) the json string into the MockReservationRequest struct
                                Ok(mock_reservation_info) => { // we got the username and password inside the login route
                                    ...
                                    match events.find_one(doc! { "_id": event_id, "is_locked": false, "is_expired": false }, None).await.unwrap(){
                                        Some(event_doc) => {
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn process_payment_request(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
