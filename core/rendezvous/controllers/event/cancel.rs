                ...
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS || access_level == DEFAULT_USER_ACCESS{ // NOTE - only dev, admin (God) and player can handle this route
                    ...
                    match serde_json::from_reader(whole_body_bytes.reader()){ // read the bytes of the filled buffer with hyper incoming body from the client by calling the reader() method from the Buf trait
                        Ok(value) => { // making a serde value from the buffer which is a future IO stream coming from the client
                            ...
                            match serde_json::from_str::<schemas::event::PlayerCancelEventRequest>(&json){ // the generic type of from_str() method is PlayerCancelEventRequest struct - mapping (deserializing) the json string into the PlayerCancelEventRequest struct
                                Ok(cancel_info) => {
