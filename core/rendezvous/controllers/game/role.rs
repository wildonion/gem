                            ...
                            match serde_json::from_str::<schemas::game::AddRoleRequest>(&json){ // the generic type of from_str() method is AddRoleRequest struct - mapping (deserializing) the json string into the AddRoleRequest struct
                                Ok(role_info) => {
                                    ...
                                    match roles.find_one(doc!{"name": role_info.clone().name}, None).await.unwrap(){
                                        Some(role_doc) => { 
                                            let response_body = misc::app::Response::<schemas::game::RoleInfo>{ // we have to specify a generic type for data field in Response struct which in our case is RoleInfo struct
                                                data: Some(role_doc),
        ...
        Ok((token_data, req)) => { // the decoded token and the request object will be returned from the function call since the Copy and Clone trait is not implemented for the hyper Request and Response object thus we can't have the borrowed form of the req object by passing it into the pass() function therefore it'll be moved and we have to return it from the pass() function   
            ...
            if middlewares::auth::user::exists(Some(&db_to_pass), _id, access_level).await{ // finding the user with these info extracted from jwt
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS || access_level == DEFAULT_USER_ACCESS{ // NOTE - only dev and admin (God) can handle this route
                    ...
                    let roles = db.clone().database(&db_name).collection::<schemas::game::RoleInfo>("roles"); // selecting roles collection to fetch and deserialize all roles infos or documents from BSON into the RoleInfo struct
                    ...
                    match roles.find(filter, None).await{
                        Ok(mut cursor) => {
                            ...
                            let response_body = misc::app::Response::<schemas::game::AvailableRoles>{
                                message: FETCHED,
