...
pub async fn add(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
    ...
    */
    match passport!{
        req,
        db.clone(),
        vec![DEV_ACCESS, ADMIN_ACCESS] // vector of access levels
    } {
        Some(passport_data) => {
                                ...
                                Ok(deck_info) => {
                                    ...
                                    if god_has_a_group || access_level == DEV_ACCESS{
                                        ...
                                        if let Some(deck_id) = deck_info.clone()._id{ // if a deck_id was passed in to the call we are sure that there is a deck info and it has a name
                                            ...
                                            resp!{
                                                schemas::game::DeckInfo, // the data type
                                                updated_deck_doc, // the data itself
                                                FOUND_DOCUMENT_UPDATE, // response message
                ...
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS{ // NOTE - only dev and admin (God) can handle this route
                    ...
                    match decks.find(filter, None).await{
                        Ok(mut cursor) => {
                            ...
                            let response_body = misc::app::Response::<schemas::game::AvailableDecks>{
                                message: FETCHED,
                ...
                if access_level == ADMIN_ACCESS || access_level == DEV_ACCESS || access_level == DEFAULT_USER_ACCESS{ // NOTE - only dev, player and admin (God) can handle this route
                    ...
                    match serde_json::from_reader(whole_body_bytes.reader()){ // read the bytes of the filled buffer with hyper incoming body from the client by calling the reader() method from the Buf trait
                        Ok(value) => { // making a serde value from the buffer which is a future IO stream coming from the client
                            let data: serde_json::Value = value;
                            let json = serde_json::to_string(&data).unwrap(); // converting data into a json string
                            match serde_json::from_str::<schemas::game::GetDeckRequest>(&json){ // the generic type of from_str() method is GetEventRequest struct - mapping (deserializing) the json string into the GetEventRequest struct
                                Ok(deck_info) => { // we got the username and password inside the login route


                                    ////////////////// DB Ops

                                    let deck_id = ObjectId::parse_str(deck_info._id.as_str()).unwrap(); // generating mongodb object id from the id string
                                    let decks = db.database(&db_name).collection::<schemas::game::DeckInfo>("decks"); // selecting decks collection to fetch and deserialize all deck infos or documents from BSON into the DeckInfo struct
                                    match decks.find_one(doc! { "_id": deck_id }, None).await.unwrap(){
                                        Some(deck_doc) => {
                                            let response_body = misc::app::Response::<schemas::game::DeckInfo>{
                                                message: FETCHED,
                                                data: Some(deck_doc),
                                                status: 200,
                                            };
                                ...
                                Ok(dis_info) => {
                                    ...
                                    match decks.find_one_and_update(doc!{"_id": deck_id}, doc!{"$set": {"is_disabled": true, "updated_at": Some(Utc::now().timestamp())}}, Some(update_option)).await.unwrap(){ // finding deck based on deck id
                                        Some(deck_doc) => { // deserializing BSON into the DeckInfo struct
                                            let response_body = misc::app::Response::<schemas::game::DeckInfo>{ // we have to specify a generic type for data field in Response struct which in our case is DeckInfo struct
                                                data: Some(deck_doc),
