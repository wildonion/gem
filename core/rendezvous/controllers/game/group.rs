...
pub async fn create(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
                                            ...
                                            if group_info.clone().god_id.unwrap() == _id.unwrap().to_string(){
                                                match groups.find_one_and_update(doc!{"name": group_info.clone().name}, doc!{
                                                    "$set": {
                                                        ...
                                                        Some(group_doc) => { 
                                                            let response_body = misc::app::Response::<schemas::game::GroupInfo>{ // we have to specify a generic type for data field in Response struct which in our case is GroupInfo struct
                                                                data: Some(group_doc),
...
pub async fn all(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
    ...
    match groups.find(None, None).await{
        Ok(mut cursor) => {
            ...
            let response_body = misc::app::Response::<schemas::game::AvailableGroups>{
                message: FETCHED,
...
// ➝ Return : Hyper Response Body or Hyper Error
// -------------------------------------------------------------------------
pub async fn god_all_groups(req: Request<Body>) -> RendezvousResult<hyper::Response<Body>, hyper::Error>{
