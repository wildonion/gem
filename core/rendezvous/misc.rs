    ...
    impl Api{
        ...
        */
        ...
        pub async fn post<F, C>(mut self, endpoint: &str, mut cb: F) -> RendezvousResult<hyper::Response<Body>, hyper::Error> // defining self (an instance of the object) as mutable cause we want to assign the name of the api; since we didn't borrow the self (the instance itself) using & we can't call this method for the second call cause the ownership of the instance will be moved in first call  
                            ...
                            C: Future<Output=RendezvousResult<hyper::Response<Body>, hyper::Error>> + Send + Sync + 'static, // C is a future object which will be returned by the closure and has bounded to Send to move across threads and .awaits
        {
