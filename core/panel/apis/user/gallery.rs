...
#[get("/gallery/get/all/i-invited/")]
#[passport(user)]
async fn get_all_galleries_i_invited(
    req: HttpRequest,
    limit: web::Query<Limit>,
    app_state: web::Data<AppState>, // shared storage (none async redis, redis async pubsub conn, postgres and mongodb)
) -> PanelHttpResponse{
        ...
        Some(pg_pool) => {
            ...
            match req.get_user(granted_role, connection).await{
                Ok(token_data) => {
                    ...
                    match UserPrivateGallery::get_all_galleries_invited(
                        &user.screen_cid.unwrap(),
                        redis_client.clone(), 
                        limit, connection).await{
...
#[get("/gallery/search/all/i-invited-to/")]
#[passport(user)]
pub(self) async fn search_in_all_galleries_invited_to(
    req: HttpRequest,
        ...
        Some(pg_pool) => {
            ...
            match req.get_user(granted_role, connection).await{
                Ok(token_data) => {
                    ...
                    match UserPrivateGallery::get_all_galleries_invited_to_without_limit(
                        &user.screen_cid.unwrap(), 
                        redis_client.clone(),
                        connection).await{
                        ...
                        Ok(mut galleries) => {
                            ...
                            let mut match_gals = vec![];
                            let search_q = &query.q;
                            galleries.retain(|gal| gal.is_some());

                            for gal in galleries{
                                let gal = gal.unwrap();
                                if gal.owner_screen_cid.contains(search_q) || 
                                    gal.owner_username.contains(search_q) || 
                                    gal.gal_name.contains(search_q){
                                        match_gals.push(Some(gal));
                                    }
                            }

                            resp!{
                                Vec<Option<UserPrivateGalleryInfoDataInvited>>, //// the data type
                                match_gals, //// response data
                                FETCHED, //// response message
                                StatusCode::OK, //// status code
                                None::<Cookie<'_>>, //// cookie
                            }
                    ...
                    if is_rate_limited!{
                        ...
                        */
                        ...
                        match UserNft::insert(
                            new_user_nft_request.clone(), 
                            redis_client.clone(), 
                            redis_actix_actor.clone(),
                            connection).await{
                            Ok(user_nft_data) => {

                                // if we inserted nft we can go for metadata and image on ipfs
                                let create_nft_metadata_uri_request = CreateNftMetadataUriRequest{ 
                                    caller_cid: new_user_nft_request.caller_cid, 
                                    amount: format!("{}", 0), 
                                    nft_new_attributes: serde_json::to_string(&new_user_nft_request.attributes).unwrap(), 
                                    nft_new_extra: serde_json::to_string(&new_user_nft_request.extra).unwrap(), 
                                    col_id: format!("{}", new_user_nft_request.col_id), 
                                    nft_id: format!("{}", user_nft_data.id), 
                                    nft_new_name: user_nft_data.nft_name, 
                                    nft_new_description: user_nft_data.nft_description, 
                                    tx_signature: new_user_nft_request.tx_signature, 
                                    hash_data: new_user_nft_request.hash_data
                                };
...
pub mod exports{
    ...
    pub use super::get_all_galleries_i_invited;
    ...
    pub use super::create_nft_with_pic;
    ...
    pub use super::search_in_all_galleries_invited_to;
}