...
/* implementing Message traits for all type of messages that can be used by EcqNotifServer actor  */
...
pub struct Connect {
    ...
    pub event_name: &'static str, // event room name: `ecq-{event_id}` to send message to and also user came to the room
...
pub struct Disconnect {
    ...
    pub event_name: String // event room name: `ecq-{event_id}` to send message to and also user disconnected from this room
}
...
pub struct Join {
    ...
    pub event_name: &'static str, // event room name: `ecq-{event_id}`
}
...
impl Actor for EcqNotifServer{

    type Context = Context<EcqNotifServer>;

    fn stopping(&mut self, ctx: &mut Self::Context) -> Running {
        
        /* 
            if the ecq notif server actor gets stopped we don't want to subscribe to redis any more
            since ecq notif server contains all rooms and sessions thus all of them MUST not receive 
...
/* handlers for all type of messages for EcqNotifServer actor to communicate with other actors */
impl Handler<JoinForPushNotif> for EcqNotifServer{
...
impl Handler<UpdateNotifRoom> for EcqNotifServer{
    ...
    fn handle(&mut self, msg: UpdateNotifRoom, ctx: &mut Self::Context) -> Self::Result{
        ...
        */
        ...
        let redis_result_rooms: RedisResult<String> = conn.get("ecq_notif_server_actor_rooms");
        let redis_rooms = match redis_result_rooms{
            ...
            Err(e) => {
                ...
                */
                ...
                let _: () = conn.set("ecq_notif_server_actor_rooms", serialized_rooms).unwrap();
                current_rooms
            }
        };
        ...
        let _: () = conn.set("ecq_notif_server_actor_rooms", serialized_rooms).unwrap(); // writing to redis ram
...
impl Handler<Disconnect> for EcqNotifServer{
...
impl Handler<Connect> for EcqNotifServer{
    ...
    fn handle(&mut self, msg: Connect, ctx: &mut Self::Context) -> Self::Result{
        ...
        info!("💡 --- current rooms of ecq notif server actor are: {:?}", self.rooms);
...
impl Handler<Join> for EcqNotifServer{ /* disconnect and connect again */
    ...
    fn handle(&mut self, msg: Join, ctx: &mut Self::Context) -> Self::Result{
        ...
        /* removing session from all rooms of EcqNotifServer actor */
        ...
        /* send disconnect message to all rooms of EcqNotifServer actor and other user */
        for room in rooms{
