...
* 🧑🏻‍💼 game managers can define score based twitter tasks for users, register new event, reveal role, collaborate with others and share their events using conse **ECQ** (Event Collaboration Queue) system and advertise their events vis SMS inside the panel  
...
* the generated cookie inside the response of the conse panel admin and user APIs is in form `<JWT>::<SHA256_OF_LOGIN_TIME>`.

* admin and user login APIs of conse panel returns a response which contains the generated cookie for the user in which we can use the first part of `::` sign, as the **JWT** to use inside the request header of the authorized APIs. 

* all conse panel APIs except admin and user login, health check and logout APIs need a **JWT** or a cookie inside the Authorization header or the cookie variable of the their request objects which the **JWT** can be set in their swagger UI page using the `Authorize 🔒` button. 
...
* conse client can subscribes to the fired or emitted role reveal, tasks and not verified events and topics by sending websocket connections to the redis server docker on the VPS.
...
* redis publish reveal role, new task and twitter task verification logs and ecq topics  
