...
* in order to reveal the roles of an event, admin **JWT** token generated by the conse mafia hyper server inside the response of the login API, must be passed to the `/admin/notif/register/reveal-role/{event_objectid}` API of the panel server. 
