...
impl Handler{
    ...
    pub async fn handle_interaction_command(mut command_queue_receiver: CommandQueueReceiver){
        ...
        tokio::spawn(async move{
            ...
            while let Some(command_data) = command_queue_receiver.recv().await{
                ...
                // ----------------------------------------------------------------------------------
                // --------------------- check user command rate limiting ---------------------------
                // ----------------------------------------------------------------------------------

                // let is_rate_limited = Handler::is_user_command_rate_limited(&ctx, &command).await;
                // if is_rate_limited == 1{
                //     if let Err(why) = command.channel_id.send_message(&ctx.http, |m|{
                //         m
                //         .content("reached rate limit")
                //         .flags(serenity::model::channel::MessageFlags::EPHEMERAL)
                //     }).await
                //     {
                //         error!("error sending message to channel since {:#?}", why);
                //     }
                // }
    ...
    async fn is_user_command_rate_limited(ctx: &Context, command: &ApplicationCommandInteraction) -> u8{
